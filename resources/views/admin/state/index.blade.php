    ...
    <div class="row mt-5 m-0">
        <div class="col-md-12">
            <div class="tile">
                ...
                <div class="col">
                    <ul>
                        <li class="active"><a href="{{ route('admin.state.index') }}">All <span class="count">({{$states->count()}})</span></a></li>
                        {{-- @php
                ...
                <div class="col-auto">
                    <form action="{{ route('admin.state.index') }}">
                    <div class="row g-3 align-items-center">
                        ...
                        <button type="submit" class="btn btn-primary">Search State</button>
                        </div>
                    </div>
                    </form>
                </div>
                            ...
                            @foreach($states as $key => $category)
                                ...
                                <td class="text-center">
                                    <div class="btn-group" role="group" aria-label="Second group">
                                        <a href="{{ route('admin.state.edit', $category['id']) }}" class="btn btn-sm btn-primary edit-btn"><i class="fa fa-edit"></i></a>
                                        <a href="{{ route('admin.state.details', $category['id']) }}" class="btn btn-sm btn-primary edit-btn"><i class="fa fa-eye"></i></a>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" action="{{route('admin.state.data.csv.store')}}" enctype="multipart/form-data" id="fileCsvUploadForm">
                        @csrf
                        ...
                        <p><a href="{{URL::to('/')}}/admin/csvexample/state.csv" target="_blank">
...
@push('scripts')
    ...
    $('.sa-remove').on("click",function(){
        ...
        function(isConfirm){
          if (isConfirm) {
            window.location.href = "state/"+id+"/delete";
            } else {
     ...
     <script type="text/javascript">
        $('input[id="toggle-block"]').change(function() {
          ...
          $.ajax({
                type:'POST',
                dataType:'JSON',
                url:"{{route('admin.state.updateStatus')}}",
